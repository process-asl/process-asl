...
""" Attempt to check each nipype interface in procasl
    Must be run in procasl directory
"""
...
class InterfaceChecker(object):
    ...
    def test_specs(self, uri):
        ...
        for c in classes:
            ...
            testdir = os.path.join('tests', 'test_interfaces')
            ...
            if not os.path.exists(nonautotest):
                with open(testfile, 'wt') as fp:
                    cmd = ['# AUTO-GENERATED by tools/checkspecs.py',
                           '# DO NOT EDIT',
                    ...
                    input_fields = ''
                    for traitname, trait in sorted(classinst.input_spec(
                            ).traits(transient=None).items()):
    ...
    for key, metadata in list(input_map.items()):
        for metakey, value in list(metadata.items()):
            ...
            for traitname, trait in sorted(classinst.input_spec().traits(
                ...
                for key in sorted(trait.__dict__):
                    ...
                    if 'parent' in trait.__dict__:
                        parent_metadata = list(getattr(
                            trait, 'parent').__dict__.keys())
                    if key not in allowed_keys +\
                            classinst._additional_metadata + parent_metadata:
                        bad_specs.append([uri, c, 'Inputs', traitname, key])
                    if key == 'mandatory' and trait.mandatory is not None and\
                            not trait.mandatory:
                        bad_specs.append([uri, c, 'Inputs', traitname,
                                          'mandatory=False'])
            ...
            if not os.path.exists(nonautotest):
                with open(testfile, 'at') as fp:
                    ...
                    input_fields = ''
                    for traitname, trait in sorted(classinst.output_spec(
                            ).traits(transient=None).items()):
                        input_fields += '%s=dict(' % traitname
                        for key, value in sorted(trait.__dict__.items()):
                            if key in in_built or key == 'desc':
